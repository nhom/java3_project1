package lv.javaguru.java3.core.database.notificationStatus;

import lv.javaguru.java3.core.database.CRUDOperationDAO;
import lv.javaguru.java3.core.database.CRUDOperationDAOImpl;
import lv.javaguru.java3.core.domain.NotificationStatus;

/**
 * Created by Vladislav on 11/26/2015.
 */
public class NotificationStatusDAOImpl extends CRUDOperationDAOImpl<NotificationStatus, Long> implements CRUDOperationDAO<NotificationStatus, Long> {
}
